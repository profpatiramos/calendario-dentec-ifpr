...
function normalize(s){
 if(!s||!Number.isInteger(s.year)||s.year<1900||s.year>9999||!['integrado','subsequente','graduacao'].includes(s.offer)||!['anual','semestral'].includes(s.regime))fail('Ano ou oferta inválidos.');
 ...
 const handle=async(req,res)=>{
  ...
  try{
   ...
   if(path==='/api/calendars'&&method==='POST'){
    const c=await store.change(db=>{const campus=db.campuses.find(c=>c.id===body.campusId);if(!campus)fail('Campus inválido.');owns({campusId:campus.id});const state=normalize({campus:campus.name,year:Number(body.year),offer:body.offer,regime:body.regime,weekdays:[],weekConfirmed:false,weekEvidence:'',periods:[],events:[]});const c={id:randomUUID(),campusId:campus.id,name:text(body.name,160),courses:text(body.courses,1000),classes:String(body.classes||'').slice(0,500),shifts:String(body.shifts||'').slice(0,200),state,version:1,versions:[],catalogueRevision:db.catalogue.revision,documentSnapshot:documentSnapshot(db,state.year),institutionalSnapshot:inheritedEvents(db.catalogue,state.year,state.offer),updatedAt:new Date().toISOString()};db.calendars.push(c);audit(db,'CREATE_CALENDAR',c.id);return c;});send(201,c);return;
   }
   ...
   if(reviewMatch){
    ...
    if(!reviewMatch[2]&&method==='PUT'){
     const support=reviewSupport(record,[...inheritedEvents(db.catalogue,record.state.year,record.state.offer),...record.state.events]);
