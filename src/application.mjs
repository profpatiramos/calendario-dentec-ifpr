...
function normalize(s){
 if(!s||!Number.isInteger(s.year)||s.year<1900||s.year>9999||!Object.keys(modalityLabels).includes(s.offer)||!['anual','semestral'].includes(s.regime))fail('Ano ou oferta inválidos.');
 ...
 return {schemaVersion:1,modalities:calendarModalities(s),assessmentStages:s.assessmentStages===undefined?undefined:Number(s.assessmentStages),campus:String(s.campus||''),year:s.year,offer:s.offer,regime:s.regime,weekdays:s.weekdays,weekEvidence:String(s.weekEvidence||'').slice(0,240),weekConfirmed:s.weekConfirmed,periods:s.periods.map(({id,name,start,end})=>({id,name,start,end})),events:s.events.map(e=>({modalities:e.modalities,requirementId:e.requirementId||undefined,id:e.id,name:e.name,start:e.start,end:e.end,kind:e.kind,evidence:e.evidence,category:eventCategory(e)}))};
}
export async function createApp({directory,setupEmail=null,files=null,store:providedStore=null,setupCode=randomBytes(16).toString('hex'),pdfRenderer=renderCalendarPdf,googleAuth=createGoogleAuth(null),publicOrigin=null}){
 ...
 const routes=new Map([['/',['web/portal.html','text/html']],['/src/modalities.mjs',['src/modalities.mjs','text/javascript']],['/evaluation.mjs',['src/evaluation.mjs','text/javascript']],['/modalities.mjs',['src/modalities.mjs','text/javascript']],['/obligations.mjs',['src/obligations.mjs','text/javascript']],['/upload.js',['web/upload.js','text/javascript']],['/documents.js',['web/documents.js','text/javascript']],['/portal.js',['web/portal.js','text/javascript']],['/ativar',['web/activate.html','text/html']],['/activate.js',['web/activate.js','text/javascript']],['/review.js',['web/review.js','text/javascript']],['/portal.css',['web/portal.css','text/css']],['/editor',['web/index.html','text/html']],['/app.js',['web/app.js','text/javascript']],['/style.css',['web/style.css','text/css']],['/calendar.mjs',['src/calendar.mjs','text/javascript']],['/saturdays.mjs',['src/saturdays.mjs','text/javascript']],['/print.mjs',['web/print.mjs','text/javascript']],['/categories.mjs',['src/categories.mjs','text/javascript']],['/src/categories.mjs',['src/categories.mjs','text/javascript']],['/ifpr-logo.png',['web/ifpr-logo.png','image/png']],['/proens.css',['web/proens.css','text/css']]]);
 const handle=async(req,res)=>{
  ...
  try{
   ...
   if(match){const id=match[1],db=(await store.read()),record=owns(db.calendars.find(c=>c.id===id));
    ...
    if(!match[2]&&method==='GET'){send(200,{...record,readiness:releaseReadiness(record,db),versions:record.versions.map(v=>({version:v.version,at:v.at,catalogueRevision:v.catalogueRevision})),institutionalEvents:inheritedEvents(db.catalogue,record.state.year,calendarModalities(record.state)),currentCatalogueRevision:db.catalogue.revision,currentDocuments:documentSnapshot(db,record.state.year),histories:db.histories.filter(h=>h.calendarId===id).map(({data,...h})=>h)});return;}
